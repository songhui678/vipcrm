...
class CActiveForm extends CWidget
{
	 ...
	 */
	public static function validate($models, $attributes=null, $loadInput=true, $isFilter=false)
	{
		$result=array();
		if(!is_array($models))
			$models=array($models);
		...
		{
			if($isFilter === true && isset($_POST[get_class($model)])){
				$model->attributes=array();
				$post = array();
				foreach ($attributes as $key => $value) {
					$post[$value] = $_POST[get_class($model)][$value];
				}
