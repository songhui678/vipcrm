...
class GerenziliaoController extends Controller
{
	/**
	...
	public function actionAdmin()
	{
		...
		// 	'model'=>$model,
		// ));
		$id = Yii::app()->controller->module->getComponent('user')->userid;
	    $model=$this->loadModel($id);
		$model->setScenario('userinfo'); 
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
		...
		if(isset($_POST['Admins']))
		{
			...
			$_POST['Admins']['password']=trim($_POST['Admins']['password']);
			$_POST['Admins']['passwordrepeat']=trim($_POST['Admins']['passwordrepeat']);
			if(!empty($_POST['Admins']['password']) or !empty($_POST['Admins']['passwordrepeat'])){
				$model->password = md5($_POST['Admins']['password']);
				$model->passwordrepeat = md5($_POST['Admins']['passwordrepeat']);
			}else{
				unset($_POST['Admins']['password']);
				unset($_POST['Admins']['passwordrepeat']);
