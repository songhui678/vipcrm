...
<tr class="even"><th>更新时间</th><td><?php echo $kehumodel->timetodate($kehumodel->update_time);?></td><th>业务人员</th><td><span style="color:red;"><?php echo $kehumodel->AdminsOne->realname;?></span></td></tr>
<tr class="odd">
	<th>微信号</th>
	<td><span><?php echo $kehumodel->wx;?></span></td>
	<th>旺旺号</th>
	<td><span><?php echo $kehumodel->wangwang;?></span></td>
</tr>
<tr class="even">
	<th>备注</th>
...
<?php } else if(Yii::app()->controller->module->getComponent('user')->cate==24){?>
  <tr class="odd"><th>取片日期</th><td><?php echo $kehumodel->timetodate2($kehumodel->taketablets_date);?></td></tr>
<?php }}  else{?> 
<tr class="odd"><td style="color:red;">此客户没有订单</td><td style="background-color:white;"><a href="/srbac/kehu/update/id/<?php echo $kehumodel->id;?>" >添加订单详情</a></td></tr>
<?php } ?> 
...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<B>添加联系记录   &nbsp;<?php if($kehumodel->remind_status==1){?> <a href="/srbac/kehu/closereminds/id/<?php echo $kehumodel->id;?>">关闭提醒</a><?php } ?></B>
	...
	<?php if(Yii::app()->controller->module->getComponent('user')->type==1){?>
		<div class="row">
			...
			<?php echo $form->dropDownList($model,'result',CHtml::listData(Record::model()->findAll("validate=1"),'id','title'),array('style'=>'width:305px;','empty'=>'-请选择-')); ?><font color="red">*</font>
	...
	<?php } else { ?>		
		<div class="row">
		<?php echo $form->labelEx($model,'result'); ?>
		<?php echo $form->dropDownList($model,'result',CHtml::listData(Record::model()->findAll("validate=1 and type=".$kehumodel->dangqian),'id','title'),array('empty'=>'-请选择-')); ?><font color="red">*</font>
		<?php echo $form->error($model,'result'); ?>
		</div>
	<?php } ?>
	...
	<div class="row">
		<label>提醒时间</label>（可以不填）
		<?php echo $form->textField($model,'remark',array('id'=>'remindtime','name'=>'remindtime','class'=>'Wdate srk','onFocus'=>"WdatePicker({isShowClear:false,readOnly:true})")); ?>
	</div>

	<div class="row buttons">
		<?php echo CHtml::submitButton($model->isNewRecord ? '提交联系记录' : '修改'); ?>
	</div>
